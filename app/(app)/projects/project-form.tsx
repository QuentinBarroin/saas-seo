...
type ProjectFormProps = {
  ...
  submitLabel?: string;
  showSeedsAndCompetitors?: boolean;
};
...
export function ProjectForm({
  ...
  action,
  defaultValues,
  submitLabel = mode === 'create' ? 'Créer le projet' : 'Enregistrer',
  showSeedsAndCompetitors = mode === 'create',
}: ProjectFormProps) {
      ...
      <div className="grid grid-cols-2 gap-4">
        <NvField label="Type" required htmlFor="type">
          <NvSelect
            ...
            name="type"
            required
            defaultValue={defaultValues?.type ?? 'saas'}
        ...
        <NvField label="Marché" htmlFor="market">
          <NvInput
            id="market"
            name="market"
            defaultValue={defaultValues?.market ?? 'FR'}
            maxLength={10}
          />
        </NvField>
      </div>
          ...
          {BUSINESS_GOAL_OPTIONS.map((g) => (
            ...
            >
              <input
                ...
                name="businessGoals"
                value={g.value}
                defaultChecked={defaultValues?.businessGoals?.includes(g.value)}
